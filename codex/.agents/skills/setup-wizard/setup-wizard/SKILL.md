...
# Setup Wizard (Codex Edition)

## When to Use

- First run (no `~/.open-workshop/config.yaml`)
...
> - **Sub-agents** (recommended): Uses Codex/agent tool. Cheap, reliable, one agent at a time. Good for most work.
> - **Agent Teams**: Multiple agents that coordinate via shared task lists. More expensive but enables true parallel work.
>
...
> **Why I'm asking:** If you have other AI CLIs installed (like OpenAI's Codex CLI or Google's Gemini CLI), the workshop can offload tasks to them to save tokens. This is optional — you can set it up later too.
...
Copy from this repo's Codex seed directory:
- `codex/seed/departments/research-and-development/knowledge.md` → `~/.open-workshop/departments/research-and-development/knowledge.md`
- `codex/seed/departments/research-and-development/tools.yaml` → `~/.open-workshop/departments/research-and-development/tools.yaml`
